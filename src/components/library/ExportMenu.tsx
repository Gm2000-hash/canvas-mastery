// Reusable export controls. `useExportActions` owns the download + Canvas
...
//
// Google Classroom will plug in here later as one more destination consuming
// the same ExportResource / resourceToHtml output.
import { useState, type ReactNode } from "react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { ChevronDown, FileSpreadsheet, FileText, FileType2, Loader2, Send } from "lucide-react";
import type { ExportResource } from "@/lib/export/resource";
import { PushToCanvasDialog } from "./PushToCanvasDialog";

export type ExportFormat = "docx" | "pdf" | "xlsx" | "canvas";
...
export function ExportMenuItems({ run, busy }: { run: (f: ExportFormat) => void; busy: ExportFormat | null }) {
