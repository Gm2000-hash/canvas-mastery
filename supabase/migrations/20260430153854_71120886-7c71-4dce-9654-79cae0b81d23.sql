-- =============================================================
-- Department analytics: relax course->subject linkage
--
-- Previously, every department aggregate required courses.discipline_id
-- to be set to a teacher_disciplines row that matched the subject. In
-- practice, Canvas-imported courses don't have discipline_id populated
-- yet, so teachers who saved their default subject in Settings still
-- saw 0 classes/students in their department dashboard.
--
-- New rule: a course belongs to subject S for teacher T if EITHER
--   (a) c.discipline_id points at a teacher_disciplines row with subject S, OR
--   (b) c.discipline_id IS NULL AND T has any teacher_disciplines row with
--       subject S (we attribute it to T's default row for that subject so
--       grade filtering still works in a sensible way).
--
-- This makes the department dashboard reflect a teacher's classes the
-- moment they pick a subject in Settings, with no extra Canvas wiring.
-- =============================================================
...
AS $$
  ...
  -- Attribute every course in the department to its subject, including
  -- courses without a discipline_id (fallback via the teacher's own discipline).
  subj_courses AS (
    SELECT DISTINCT c.id AS course_id, c.teacher_id, td.subject
    FROM public.courses c
    JOIN public.teacher_disciplines td
      ON td.subject IN (SELECT subject FROM my_subjects)
     AND (
          (c.discipline_id IS NOT NULL AND c.discipline_id = td.id)
       OR (c.discipline_id IS NULL AND td.teacher_id = c.teacher_id)
     )
    WHERE c.archived_at IS NULL
      AND (_school_year IS NULL
  ...
  SELECT
    ...
    (SELECT COUNT(DISTINCT sc.course_id)::int
       FROM subj_courses sc
       WHERE sc.subject = ms.subject
         AND sc.teacher_id IN (SELECT teacher_id FROM public.department_membership(ms.subject, NULL, _school_year))),
    (SELECT COUNT(*)::int
       FROM public.students st
       JOIN subj_courses sc ON sc.course_id = st.course_id AND sc.teacher_id = st.teacher_id
       WHERE sc.subject = ms.subject
...
-- Helper: build the (course_id, teacher_id, grade, name) "scope" for a subject.
-- Used by the per-subject RPCs below.
CREATE OR REPLACE FUNCTION public.department_scope_courses(
  _subject text,
  _grades text[] DEFAULT NULL,
  _school_year text DEFAULT NULL
)
RETURNS TABLE(course_id uuid, teacher_id uuid, name text, grade text)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
  WITH members AS (
    SELECT m.teacher_id
    FROM public.department_membership(_subject, _grades, _school_year) m
  )
  SELECT DISTINCT ON (c.id)
    c.id, c.teacher_id, c.name, td.grade
  FROM public.courses c
  JOIN public.teacher_disciplines td
    ON td.subject = _subject
   AND (
        (c.discipline_id IS NOT NULL AND c.discipline_id = td.id)
     OR (c.discipline_id IS NULL AND td.teacher_id = c.teacher_id)
   )
  WHERE c.archived_at IS NULL
    ...
    AND (_school_year IS NULL
         OR public.school_year_label(COALESCE(c.end_at, c.last_synced_at, c.created_at)) = _school_year)
  ORDER BY c.id, (td.teacher_id = c.teacher_id) DESC, td.is_default DESC, td.created_at;
$$;

REVOKE ALL ON FUNCTION public.department_scope_courses(text, text[], text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.department_scope_courses(text, text[], text) TO authenticated;
...
BEGIN
  ...
  WITH scope_courses AS (
    SELECT * FROM public.department_scope_courses(_subject, _grades, _school_year)
  ),
  scope_students AS (
