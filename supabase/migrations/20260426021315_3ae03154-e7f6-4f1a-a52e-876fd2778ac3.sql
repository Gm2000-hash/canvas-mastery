...
CREATE OR REPLACE FUNCTION public.analytics_student_breakdown(_course_id uuid DEFAULT NULL::uuid)
...
CREATE OR REPLACE FUNCTION public.analytics_standard_breakdown(_course_id uuid DEFAULT NULL::uuid, _subject text DEFAULT NULL::text, _framework text DEFAULT NULL::text)
RETURNS TABLE(standard_id uuid, code text, description text, subject text, grade text, framework text, students_assessed integer, students_mastered integer, avg_mastery numeric, pct_mastered numeric)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
  ...
  LEFT JOIN latest_ms lms ON lms.standard_id = s.id
  LEFT JOIN public.students st ON st.id = lms.student_id
  WHERE (s.teacher_id = auth.uid() OR s.teacher_id IS NULL)
    ...
    AND (_course_id IS NULL OR st.course_id = _course_id)
  GROUP BY s.id
  HAVING (_course_id IS NULL AND COUNT(lms.standard_id) > 0) OR (_course_id IS NOT NULL AND COUNT(st.id) > 0)
  ORDER BY ROUND(AVG(CASE WHEN lms.mastered THEN 1 ELSE 0 END)::numeric, 4) NULLS LAST, s.code;
$$;

CREATE OR REPLACE FUNCTION public.analytics_mastery_distribution(_course_id uuid DEFAULT NULL::uuid, _subject text DEFAULT NULL::text)
...
AS $$
  ...
  latest_ms AS (
    SELECT DISTINCT ON (ms.student_id, ms.standard_id)
      ms.student_id, ms.standard_id, ms.mastery_score
    FROM public.mastery_snapshots ms
    WHERE ms.teacher_id = auth.uid()
    ORDER BY ms.student_id, ms.standard_id, ms.computed_at DESC
  ),
  data AS (
    SELECT lms.mastery_score
    FROM latest_ms lms
    JOIN public.students st ON st.id = lms.student_id
    JOIN public.standards s ON s.id = lms.standard_id
    WHERE st.teacher_id = auth.uid()
      AND (_course_id IS NULL OR st.course_id = _course_id)
      AND (_subject IS NULL OR s.subject = _subject)
  )
...
CREATE OR REPLACE FUNCTION public.analytics_assignment_breakdown(_course_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(assignment_id uuid, name text, course_id uuid, course_name text, kind text, due_at timestamp with time zone, points_possible numeric, submission_count integer, avg_percentage numeric, standards_tagged integer)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
  SELECT a.id, a.name, c.id, c.name, a.kind::text, a.due_at, a.points_possible,
    ...
    (SELECT COUNT(*)::int FROM public.assignment_standards asg WHERE asg.assignment_id = a.id AND (asg.confirmed OR asg.ai_suggested))
...
CREATE OR REPLACE FUNCTION public.analytics_question_breakdown(_assignment_id uuid DEFAULT NULL::uuid, _course_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(question_id uuid, assignment_id uuid, assignment_name text, question_position integer, question_text text, points_possible numeric, responses integer, correct_count integer, pct_correct numeric, avg_points numeric, standards_tagged integer)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
  SELECT q.id, q.assignment_id, a.name, q.position, q.question_text, q.points_possible,
    ...
    (SELECT ROUND(AVG(r.points)::numeric, 2)
     FROM public.question_responses r WHERE r.question_id = q.id),
    (SELECT COUNT(*)::int FROM public.question_standards qs WHERE qs.question_id = q.id AND (qs.confirmed OR qs.ai_suggested))
...
CREATE OR REPLACE FUNCTION public.analytics_mastery_trends(_course_id uuid DEFAULT NULL::uuid, _subject text DEFAULT NULL::text, _granularity text DEFAULT 'week'::text)
RETURNS TABLE(bucket_label text, bucket_ts timestamp with time zone, framework text, subject text, avg_mastery numeric, sample_size integer)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
  ...
  per_asg AS (
    ...
    JOIN public.assignment_standards asg ON asg.assignment_id = a.id AND (asg.confirmed = true OR asg.ai_suggested = true)
